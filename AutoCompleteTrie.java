...
public class AutoCompleteTrie implements Trie {
	...
	private double[] freqs = new double[] { 8.2, 1.5, 2.8, 4.3, 12.7, 2.2, 2.0, 6.1, 7.0, 0.15,
	...
	private TrieNode probableChar2(boolean terminal) {
		...
		int k = 1;
		for (Integer i : possibles) { 
			double hold = Math.round((freqs[i] / adjustedAmount) * 10000);
			hold = hold / 100;
		...
		double randomprob = rand.nextDouble(adjusted[adjusted.length-1]);
	...
	// Method takes into account the frequency of each character in English and determines
	...
	// 		// if random is between a range, return that TrieNode associated with the character
